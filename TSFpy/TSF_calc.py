...
TSF_calc_okusenman="垓京兆億万千百十銭"
TSF_calc_okusenzero=["*1"+'0'*20+"+","*1"+'0'*16+"+","*1"+'0'*12+"+","*1"+'0'*8+"+","*1"+'0'*4+"+","1000+","100+","10+","+1/100"]
...
TSF_calc_usemark="1234567890.|pmyecn+-*/\\#%(S!LG~)"
...
def TSF_calc_stackmarge(TSF_calcQ):
    ...
    for TSF_calcbracketK in TSF_calcK:
        TSF_calcbracketK=TSF_calc_operator.get(TSF_calcbracketK,TSF_calcbracketK)
        TSF_calcA+=TSF_calcbracketK if TSF_calcbracketK in TSF_calc_usemark else ''
    return TSF_calcA

def TSF_calc_bracketsbalance(TSF_calcQ):
    TSF_calcQ=TSF_calcQ.replace('\n','\t').replace('\t','')
    TSF_calcQ=TSF_calc_stackmarge(TSF_calcQ)
    TSF_calcA=TSF_calcQ; TSF_calcbracketLR,TSF_calcbracketCAP=0,0
    for TSF_calcbracketQ in TSF_calcQ:
        if TSF_calcbracketQ == '(':
            TSF_calcbracketLR+=1
        if TSF_calcbracketQ == ')':
...
def TSF_calc_debug(TSF_argv=[]):    #TSF_doc:「TSF/TSF_calc.py」単体テスト風デバッグ関数。
    ...
    TSF_debug_log=TSF_io_printlog("TSF_calc_bracketsbalance:",TSF_log=TSF_debug_log)
    LTsv_calcQlist=[ "1/3)｛｝","(1/3*3","(1|3*3)","1|3","{1}+{2}"]
    for LTsv_calcQ in LTsv_calcQlist:
        TSF_debug_log=TSF_io_printlog("\t{0}⇔{1}".format(LTsv_calcQ,TSF_calc_bracketsbalance(LTsv_calcQ)),TSF_debug_log)
#    TSF_debug_log=TSF_io_printlog("\t{0}".format(TSF_calc_bracketsbalance("()")),TSF_log=TSF_debug_log)
