...
def TSF_command_about(save_about_mergefile):    #TSF_doc:TSFの概要とサンプルプログラム。
    ...
    TSF_Forth_settext("aboutTSF:",
        ...
        ,TSF_style="N")
    TSF_Forth_settext("calcQQtest:","\t".join(["「1/3+1|2」→","1/3+1|2","#TSF_calcQQ","2","#TSF_join","#TSF_echo"]))
    TSF_Forth_settext("calcFXtest:","\t".join(["「1 3 1|2」を数式風に「[2]/[1]+[0]」で連結→","1","3","1|2","[2]/[1]+[0]","#TSF_calc[]","#TSF_calcFX","2","#TSF_join","#TSF_echo"]))
    TSF_Forth_settext("calcDCtest:","\t".join(["「1 / 3 + 1|2」まで分解して連結(ついでに小数デモ)→","1","/","3","+","1|2","5","#TSF_join","#TSF_calcDC","2","#TSF_join","#TSF_echo"]))
    TSF_Forth_settext("calc日本語風:","\t".join(["日本語風に「一割る三足す二分の一」→","一割る三足す二分の一","#TSF_calcFX","2","#TSF_join","#TSF_echo"]))
