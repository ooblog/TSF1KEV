...
def TSF_sample_about():    #TSF_doc:TSFの概要サンプルプログラム。
    TSF_Forth_setTSF(TSF_Forth_1ststack(),"\t".join(["UTF-8","#TSF_encoding","30","#TSF_calcPR","main1:","#TSF_this","0","#TSF_fin."]))
    TSF_Forth_setTSF("main1:","\t".join(["aboutTSF:","#TSF_echothe","main2:","#TSF_this"]))
    TSF_Forth_setTSF("main2:","\t".join(["#-- 分岐の材料、電卓その他数値取得のテスト --"," ","2","#TSF_echoN",
        "calcFXtest:","#TSF_this","calcDCtest:","#TSF_this","calcKNテスト:","#TSF_this",
        "calenderテスト:","#TSF_this","matchテスト:","#TSF_this","shuffleテスト:","#TSF_this",
        " ","#-- (小数の桁数が異なる理由は分数電卓を用いない計算は有効桁数が短いため) --","2","#TSF_echoN","main3:","#TSF_this"]))
    TSF_Forth_setTSF("main3:","\t".join(["aboutCalc:","#TSF_echothe"]))
    ...
    TSF_Forth_setTSF("calcDCtest:","\t".join(["「1 / 3 - m1|2」を数式に連結(ついでに小数30桁デモ)→","1","/","3","-","m1|2","5","#TSF_joinN","#TSF_calcDC","2","#TSF_joinN","1","#TSF_echoN"]))
    TSF_Forth_setTSF("calcKNテスト:","\t".join(["「一割る三引く(マイナス二分の一)」(ついでに単位付き計算デモ)→","一割る三引く(マイナス二分の一)","#単位計算","2","#N個連結","1","#N行表示"]))
    TSF_Forth_setTSF("calenderテスト:","\t".join(["「@bt」SwatchBeat(スイスから時差8時間)→","-480","#TSF_diffminute","@bt","#TSF_calender","2","#TSF_joinN","1","#TSF_echoN"]))
    TSF_Forth_setTSF("matchテスト:","\t".join(["「いいまちがい」と「いいまつがい」の類似度→","いいまちがい","いいまつがい","#TSF_matcher","2","#TSF_joinN","1","#TSF_echoN"]))
    TSF_Forth_setTSF("shuffleテスト:","\t".join(["前半TSF概要の行数(スタック「aboutTSF:」の個数)→","aboutTSF:","#TSF_lenthe","2","#TSF_joinN","1","#TSF_echoN"]))
